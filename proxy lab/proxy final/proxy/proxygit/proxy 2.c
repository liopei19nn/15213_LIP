...
//    char* host_original;
...
#ifdef DEBUG    
    dbg_printf("process_request : start\n");
    ...
    dbg_printf("process_request : Gethostbyaddr success !\n");
...
//    host = Malloc(sizeof(char) * MAX_DNLEN);
//    host_original = host;
...
//    *port = 80;
    ...
    dbg_printf("process request : [host : %s][port : %d]\n",host,port);

//    proxy_req = Malloc(MAXLINE * sizeof(char));
 
    sprintf(proxy_req,"GET %s HTTP/1.0\r\n",modify_uri(uri));
    dbg_printf("process_request : REQ without headers :\n%s",proxy_req);

    generate_request(proxy_req,&rio);

    dbg_printf("Request generated :\n%s",proxy_req);        
