...
void serve(int file_d,struct sockaddr_in* clientaddrp);
...
int main(int argc, char **argv)
{
    ...
    while(1)
    {
        ...
        threadInfo = Malloc(sizeof(struct threadInfo_t));
        ...
        threadInfo -> connfd = Accept(listenfd,(SA *)&threadInfo -> clientaddr,(socklen_t *)&clientlen);
        // thread((void *)threadInfo);
        //process_request(connfd,clientaddr);
        Pthread_create(&tid, NULL, doit, (void *)threadInfo);
...
void *doit(void *arg)
{
  ...
  threadInfo = (struct threadInfo_t*)arg;

  int connfd = threadInfo -> connfd;

  struct sockaddr_in* clientaddrp = &threadInfo -> clientaddr;
...
*/
 void serve(int file_d,struct sockaddr_in* clientaddrp)
 {
    ...
    {
        clienterror(file_d,method,"501","Not Implemented","Proxy does not implement this method");       
        return;
