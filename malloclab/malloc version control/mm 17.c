 ...
 * malloc(size): Get size, add header,footer with size
 to get asize (aligned size), search for free block
 according to the azise and alignment in free block
 lists
 
 * free(*bp): Get a block reference pointer and free this
 block
...
/*
 * Method Specifics:
 * Segregated List, Mixed first-fit and best-fit
 */
...
void mm_checkheap(int verbose)
{
    ...
    for (int i = 1; i <= NUM_OF_LIST; i++) {
        ...
        for (bp = heap_listp + i * DSIZE; GET_ALLOC(HDRP(bp))==0; bp = PREV_FREEP(bp))
        {
            if (verbose)
                printBlock(bp);
            checkBlock(bp);
