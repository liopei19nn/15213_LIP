...
#define NUM_OF_LIST 18
#define INITIAL_SIZE 160
...
int mm_init(void)
{
	...
	if ((heap_listp = mem_sbrk(INITIAL_SIZE)) == NULL)
		return -1;
...
static void *findFit(size_t asize)
{
     ...
     */
    head = PREV_FREEP(heap_listp + offset * DSIZE);
...
static void insertAtFront(void *bp)
{
    ...
    void *head = PREV_FREEP(heap_listp + offset * DSIZE);
    
    if (head == NULL) {
        PREV_FREEP(heap_listp + offset * DSIZE) = bp;
        NEXT_FREEP(bp) = heap_listp + offset * DSIZE;
        PREV_FREEP(bp) = NULL;
        return;
    }else{
        PREV_FREEP(heap_listp + offset * DSIZE) = bp;
